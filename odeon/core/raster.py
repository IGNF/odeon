...
from .rio_utils import get_scale_factor_and_img_size_from_dataset, get_center_from_bound
from .rio_utils import get_bounds, create_patch_from_center


def get_img(path, indexes=None):

    path = os.path.join(path)
    indexes = [1, 2, 3] if indexes is None else indexes
...
def raster_to_ndarray_from_dataset(src,
    ...
    """Load and experimental_test_transform_kornia an image into a ndarray according to parameters:
    ...
    " compute the new affine experimental_test_transform_kornia "
    if scaled_width != width or scaled_height != height:
        ...
        meta["experimental_test_transform_kornia"] = affine
...
def raster_to_ndarray(image_file,
    ...
    Parameters
    ----------
    image_file : rasterio.DatasetReader
        raster source for the conversion
    width : int, optional
        output image width, by default None (native image width is used)
    height : int, optional
    ...
    with rio.open(image_file) as src:
        ...
        if height is None:
            height = src.height
        out = raster_to_ndarray_from_dataset(src, width, height, resolution, band_indices, resampling, window)
    return out
...
def ndarray_to_affine(affine):
    ...
    rasterio.Affine
    """
    return rio.Affine(affine[0], affine[1], affine[2], affine[3], affine[4], affine[5])
...
class CollectionDatasetReader:
    ...
    def get_stacked_window_collection(dict_of_raster,
        ...
        for key, value in dict_of_raster.items():
            if (key not in ["DSM", "DTM"]) or handle_dem is False:
                ...
                window = from_bounds(bounds[0], bounds[1], bounds[2], bounds[3], src.meta["experimental_test_transform_kornia"])
        ...
        if handle_dem:
            ...
            dsm_window = from_bounds(bounds[0], bounds[1], bounds[2], bounds[3], dsm_ds.meta["experimental_test_transform_kornia"])
            dtm_window = from_bounds(bounds[0], bounds[1], bounds[2], bounds[3], dtm_ds.meta["experimental_test_transform_kornia"])
    ...
    def stack_window_raster(center,
        ...
        window = from_bounds(bounds[0], bounds[1], bounds[2], bounds[3], meta['experimental_test_transform_kornia'])

        # Adapt meta for the patch
        meta_img_patch = meta.copy()
        meta_img_patch['experimental_test_transform_kornia'] = transform(window, meta['experimental_test_transform_kornia'])

        meta_msk_patch = meta_msk.copy()
        meta_msk_patch["experimental_test_transform_kornia"] = transform(window, meta_msk['experimental_test_transform_kornia'])
        ...
        if not compute_only_masks:
            ...
            with rio.open(center["img_file"], 'w', **meta_img_patch) as dst:
