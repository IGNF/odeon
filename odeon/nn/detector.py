...
class BaseDetector:
    def __init__(
        ...
        self.model = BaseDetector.load_model(
            self.model_name,
            self.model_path,
            self.n_channel,
            self.n_classes,
        ...
        if use_cuda:
            if self.idx_gpu is None and self.use_gpu is True:
                ...
                if self.idx_gpu in list_gpus:
                    ...
                    LOGGER.error(
                        f"ERROR: Input GPU reference doesn't exist in available GPU devices: {list_gpus}"
                    )
                    raise OdeonError(
                        message=f"Input GPU reference cuda:{self.idx_gpu} doesn't exist",
    ...
    def load_model(model_name, model_path, n_channel, n_classes, device="cpu"):
        if model_name not in model_list:
            raise OdeonError(
                message=f"the model name {model_name} does not exist",
...
class PatchDetector(BaseDetector):
    ...
    def configure(self):
        ...
        self.dataset = PatchDetectionDataset(
            self.job,
        ...
        for prediction, index, affine in zip(predictions, indices, affines):
            ...
            with rasterio.open(
                ...
                prediction = (
                    ...
                    .convert(prediction, threshold=self.threshold)
                )
...
class ZoneDetector(PatchDetector):
    ...
    def configure(self):
        ...
        if self.out_dalle_size is None:
            ...
            self.meta_output["height"] = self.img_size_pixel * self.tile_factor - (
                2 * self.margin_zone
            )
    ...
    def save(self, predictions, indices):
        ...
        for prediction, index in zip(predictions, indices):
            ...
            prediction = substract_margin(
                prediction, self.margin_zone, self.margin_zone
            )
            ...
            if (
                self.out_dalle_size is not None
                and self.rio_ds_collection.collection_has_key(output_id)
            ):
                ...
                window = geometry_window(
                    self.dst, [geometry], pixel_precision=6
                ...
                out = rasterio.open(
                    output_file, "w+", **self.meta_output, **self.gdal_options
                )
            ...
            out.write_band(
                [i for i in range(1, self.n_classes + 1)], prediction, window=window
            )

            self.job.set_cell_at(index[0], "job_done", 1)

            if self.out_dalle_size is not None and self.job.job_finished_for_output_id(
                output_id
            ):
