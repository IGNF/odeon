...
def build_model(model_name, n_channels, n_classes, deterministic=False):

    bilinear = False if deterministic else True
    if deterministic and model_name not in ["unet", "lightunet"]:
        LOGGER.error(
            "ERROR: The reproductibility of a training only works for Unet and Lightunet models in this version."
        )
        raise OdeonError(
            ErrorCodes.ERR_MODEL_ERROR,
            "Wrong parameters model_name or reproducible. \
                If reproducible is True model name should be 'unet' or 'lightunet'",
        )
...
def save_model(
    out_dir, out_filename, model, optimizer=None, scheduler=None, train_dict=None
):
