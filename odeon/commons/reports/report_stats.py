...
class Report_Stats(Report):
    ...
    def rounded_stats(self):
        ...
        """
        self.input_object.df_bands_stats = self.round_df_values(
            self.input_object.df_bands_stats
        )
        self.input_object.df_classes_stats = self.round_df_values(
            self.input_object.df_classes_stats
        )
        self.input_object.df_global_stats = self.round_df_values(
            self.input_object.df_global_stats
        )
    ...
    def to_json(self):
        ...
        with open(
            os.path.join(self.input_object.output_path, "stats_report.json"), "w"
        ) as output_file:
...
![Histograms radio {class_name}](./{os.path.basename(os.path.basename(self.path_radios[class_name]))})"""
        ...
        with open(
            os.path.join(self.input_object.output_path, "stats_report.md"), "w"
        ) as output_file:
    ...
    def to_html(self):
        ...
        with open(
            os.path.join(self.input_object.output_path, "stats_report.html"), "w"
        ) as output_file:
