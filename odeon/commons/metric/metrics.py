...
NBR_METRICS_PER_CLASS = len(
    ["Accuracy", "Precision", "Recall", "Specificity", "F1-Score", "IoU"]
)
...
def get_metrics_from_obs(
    true_pos, false_neg, false_pos, true_neg, smooth=SMOOTH, micro=False
):
    """
    ...
    if micro:
        ...
        metrics = {"OA": oa, "IoU": iou}
    else:
        accuracy = (true_pos + true_neg) / (
            true_pos + false_pos + true_neg + false_neg + smooth
        )
...
class Metrics(ABC):
    ...
    def __init__(
        ...
        if output_type in ["md", "json", "html"]:
            ...
            raise OdeonError(
                ErrorCodes.ERR_JSON_SCHEMA_ERROR, "The input output type is incorrect."
            )
        ...
        if weights is None:
            ...
            if self.type_classifier == "binary":
                LOGGER.warning(
                    "WARNING: the parameter weigths can only be used for multiclass classifier."
                )
                self.weighted = False
                self.weights = None
            else:
        ...
        self.decimals = (
            2 + decimals
        )  # Here + 2 because we wants metrics as percent between 0 and 100.
        ...
        if self.threshold not in self.threshold_range:
            self.threshold_range = np.sort(
                np.append(self.threshold_range, self.threshold)
            )

        if self.output_type == "json":
            self.dict_export["params"] = {
    ...
    def define_bins(self, bins, n_bins):
        ...
        if self.n_bins <= 20:
            self.bins_xticks = [
                np.round(bin_i, decimals=decimals) for bin_i in self.bins
            ]
        else:
            self.bins_xticks = [
                np.round(bin_i, decimals=decimals)
                for bin_i in np.linspace(0.0, 1.0, 11)
            ]
    ...
    def get_confusion_matrix(truth, pred, nbr_class, revert_order=True):
        ...
        """
        assert isinstance(truth, (np.ndarray, np.generic)) and isinstance(
            pred, (np.ndarray, np.generic)
        )
        ...
        for i, class_i in enumerate(class_ids):
            for j, class_j in enumerate(class_ids):
                conf_mat[i, j] = np.sum(
                    np.logical_and(truth == class_i, pred == class_j)
                )
        if revert_order:
