...
def heatmap(
    data, row_labels, col_labels, axes=None, cbar_kw=None, cbarlabel="", **kwargs
):
    """
            ...
            for j in range(data.shape[1]):
                ...
                if valfmt[i, j] == "nodata":
                    ...
                    text = image.axes.text(
                        ...
                        str(
                            np.round(
                                data[i, j] / valfmt[i, j][0] if data[i, j] != 0 else 0,
                                decimals,
                            )
                        )
                        + valfmt[i, j][1],
...
def plot_confusion_matrix(
    ...
    image, _ = heatmap(
        conf_mat, labels, labels, axes=axes, cmap=cmap, cbarlabel=cbarlabel
    )
...
def plot_norm_and_value_cms(
    conf_mat, labels, output_path=None, per_class_norm=True, style=None, cmap="YlGn"
):
    ...
    im0, _ = heatmap(
        cm_norm, labels, labels, axes=axs[1], cmap=cmap, cbarlabel=cbarlabel
    )
    ...
    if not per_class_norm:
        ...
        axs[1].set_title(
            "Normalized per actual class values", y=-0.1, pad=-14, fontsize=fontsize
        )

    im1, _ = heatmap(
        conf_mat, labels, labels, axes=axs[0], cmap=cmap, cbarlabel=cbarlabel
    )
    cm_val_fmt = get_cm_val_fmt(conf_mat)
...
def prepare_data_pr_curve(data):
    ...
    precision = np.array(
        [1 if p == 0 and r == 0 else p for p, r in zip(precision, recall)]
    )
...
def plot_roc_pr_curves(data, labels, nbr_class, output_path, decimals):
    ...
    """
    cmap_colors = [
        plt.get_cmap("rainbow")(1.0 * i / nbr_class) for i in range(nbr_class)
    ]
    ...
    for class_i, color in zip(labels, colors):
        fpr, tpr, roc_auc = prepare_data_roc_curve(
            data[class_i] if nbr_class > 1 else data
        )
        plt.plot(
            fpr,
            tpr,
    ...
    plt.subplot(122)
    for class_i, color in zip(labels, colors):
        precision, recall, pr_auc = prepare_data_pr_curve(
            data[class_i] if nbr_class > 1 else data
        )
        plt.plot(
            recall,
...
def plot_calibration_curves(
    prob_true, prob_pred, hist_counts, labels, nbr_class, output_path, bins, bins_xticks
):
    """
    ...
    for class_i, color in zip(labels, colors):
        plt.plot(
            prob_true[class_i] if nbr_class > 1 else prob_true,
...
def plot_hists(
    ...
    cmap_colors = [plt.get_cmap("turbo")(1.0 * i / 7) for i in range(7)][1:]
    colors = cycler(color=cmap_colors)
    ...
    for i, plot_prop in enumerate(zip(list_metrics, colors)):
        ...
        plt.bar(
            range(len(values)), values, width=0.8, linewidth=2, capsize=20, color=color
        )
        if n_bins <= 20:
