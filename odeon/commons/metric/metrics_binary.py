...
class MetricsBinary(Metrics):
    ...
    def create_data_for_metrics(self):
        ...
        df_thresholds = pd.DataFrame(
            index=range(len(self.threshold_range)),
            columns=(["threshold"] + self.metrics_names),
        )
        df_thresholds["threshold"] = self.threshold_range
        cms = {
            threshold: np.zeros([self.nbr_class, self.nbr_class])
            for threshold in self.threshold_range
        }
        df_report_metrics = pd.DataFrame(
            index=["Values"], columns=self.metrics_names[:-1]
        )
    ...
    def get_metrics_by_threshold(self):
        ...
        for dataset_index, sample in enumerate(
            ...
            if self.get_metrics_per_patch or self.get_hists_per_metrics:
                pred_cm = self.binarize(
                    self.type_classifier, pred, threshold=self.threshold
                )
                conf_mat = self.get_confusion_matrix(
                    mask.flatten(), pred_cm.flatten(), self.nbr_class, revert_order=True
                )
                ...
                for name_column in self.metrics_names[:-1]:
                    self.df_dataset.loc[dataset_index, name_column] = sample_metrics[
                        name_column
                    ]
            ...
            if self.get_calibration_curves:
                ...
                bin_sums += np.bincount(
                    binids, weights=pred.flatten(), minlength=len(self.bins)
                )
                # Bins counts of indices times the values of the masks.
                bin_true += np.bincount(
                    binids, weights=mask.flatten(), minlength=len(self.bins)
                )
        ...
        for threshold in self.threshold_range:
            ...
            for metric, metric_value in cr_metrics.items():
                self.df_thresholds.loc[
                    self.df_thresholds["threshold"] == threshold, metric
                ] = metric_value
        ...
        if self.get_ROC_PR_curves or self.get_ROC_PR_values:
            self.vect_curves = {
                "TPR": self.df_thresholds["Recall"].tolist(),
        ...
        self.df_report_metrics.loc["Values"] = self.df_thresholds.loc[
            self.df_thresholds["threshold"] == self.threshold, self.metrics_names[:-1]
        ].values
    ...
    def export_values(self):
        ...
        if self.get_ROC_PR_values:
            ...
            df_roc_pr_values = pd.DataFrame(
                data={
                    "Thresholds": self.threshold_range,
