...
def torch_metrics_from_cm(cm_macro, class_labels):
    ...
    for metric_name, metric_per_class in metrics_by_class.items():
        ...
        for class_idx, class_label in enumerate(class_labels):
            metrics_collection[class_label + "/" + metric_name] = (
                metric_per_class[class_idx] * 100
            )
...
def get_metrics_from_cm(cm_macro, nbr_class, class_labels, weighted, weights):
    ...
    obs_by_class = get_obs_by_class_from_cm(
        conf_mat=cm_macro, class_labels=class_labels
    )
    cms_classes = np.zeros([nbr_class, 2, 2])

    metrics_by_class = {}
    for i, class_i in enumerate(class_labels):
        cms_classes[i] = np.array(
            [
                [obs_by_class[class_i]["tp"], obs_by_class[class_i]["fn"]],
                [obs_by_class[class_i]["fp"], obs_by_class[class_i]["tn"]],
            ]
        )
        metrics_by_class[class_i] = get_metrics_from_obs(
            obs_by_class[class_i]["tp"],
    ...
    metrics_micro = get_metrics_from_obs(
        cm_micro[0][0], cm_micro[0][1], cm_micro[1][0], cm_micro[1][1]
    )
...
class MetricsMulticlass(Metrics):
    ...
    def __init__(
        ...
        self.vect_classes = {}
        (
            self.df_report_classes,
            self.df_report_micro,
            self.df_report_macro,
        ) = self.create_data_for_metrics()

        # Dicts to store info for calibrations curves.
        self.dict_prob_true, self.dict_prob_pred = {}, {}
        self.dict_hist_counts = {
            class_i: np.zeros(len(self.bins) - 1) for class_i in self.class_labels
        }
        self.dict_bin_sums = {
            class_i: np.zeros(len(self.bins)) for class_i in self.class_labels
        }
        self.dict_bin_true = {
            class_i: np.zeros(len(self.bins)) for class_i in self.class_labels
        }
    ...
    def create_data_for_metrics(self):
        ...
        """
        df_report_classes = pd.DataFrame(
            index=self.class_labels, columns=self.metrics_names[:-1]
        )
        df_report_micro = pd.DataFrame(index=["Values"], columns=["OA", "IoU"])
        if self.weighted:
            df_report_macro = pd.DataFrame(
                index=["Average", "Weighted avg", "User weighted avg"],
                columns=["Precision", "Recall", "F1-Score", "IoU"],
            )
        else:
            df_report_macro = pd.DataFrame(
                index=["Average", "Weighted avg"],
                columns=["Precision", "Recall", "F1-Score", "IoU"],
            )
    ...
    def scan_dataset(self):
        ...
        self.cms_one_class = pd.DataFrame(
            index=self.threshold_range, columns=self.class_labels, dtype=object
        )
        self.cms_one_class = self.cms_one_class.applymap(lambda x: np.zeros([2, 2]))

        for index_sample, sample in enumerate(
            tqdm(self.dataset, desc="Metrics processing time", leave=True)
        ):
            ...
            mask_macro, pred_macro = self.binarize(
                type_classifier=self.type_classifier, prediction=pred, mask=mask
            )
            ...
            for i, class_i in enumerate(self.class_labels):
                ...
                if self.get_ROC_PR_curves or self.get_ROC_PR_values:
                    for threshold in self.threshold_range:
                        bin_pred = self.binarize(
                            "binary", class_pred, threshold=threshold
                        )
                        cr_cm = self.get_confusion_matrix(
                            class_mask.flatten(),
                            bin_pred.flatten(),
                            nbr_class=2,
                            revert_order=True,
                ...
                if self.get_calibration_curves:
                    ...
                    self.dict_hist_counts[class_i] += np.histogram(
                        class_pred.flatten(), bins=self.bins
                    )[0]
                    ...
                    self.dict_bin_sums[class_i] += np.bincount(
                        binids, weights=class_pred.flatten(), minlength=len(self.bins)
                    )
                    # Bins counts of indices times the values of the masks.
                    self.dict_bin_true[class_i] += np.bincount(
                        binids, weights=class_mask.flatten(), minlength=len(self.bins)
                    )
                    # Total number observation per bins.
                    self.dict_bin_total[class_i] += np.bincount(
                        binids, minlength=len(self.bins)
                    )

        if self.get_calibration_curves:
            for class_j in self.class_labels:
                nonzero = (
                    self.dict_bin_total[class_j] != 0
                )  # Avoid to display null bins.
                # The proportion of samples whose class is the positive class, in each bin (fraction of positives).
                prob_true = (
                    self.dict_bin_true[class_j][nonzero]
                    / self.dict_bin_total[class_j][nonzero]
                )
                # The mean predicted probability in each bin.
                prob_pred = (
                    self.dict_bin_sums[class_j][nonzero]
                    / self.dict_bin_total[class_j][nonzero]
                )
            ...
            self.dict_hist_counts = {
                key: value / total_pixel for key, value in self.dict_hist_counts.items()
            }

        if self.get_ROC_PR_curves or self.get_ROC_PR_values:
            self.cms_one_class = self.cms_one_class.applymap(
                lambda cm_one_class: get_metrics_from_obs(
                    cm_one_class[0][0],
                    cm_one_class[0][1],
                    cm_one_class[1][0],
                    cm_one_class[1][1],
        ...
        if self.get_hists_per_metrics:
            for metric in self.header[1:]:
                self.hists_metrics[metric] = np.histogram(
                    list(self.df_dataset.loc[:, metric]), bins=self.bins
                )[0].tolist()
    ...
    def compute_metrics_per_patch(self, conf_mat_patch, index_patch, name_patch):
        ...
        name_patch : str
            Path to the patch.
        """
        metrics_by_class, metrics_micro, _, _ = get_metrics_from_cm(
            conf_mat_patch,
            nbr_class=self.nbr_class,
            class_labels=self.class_labels,
            weighted=self.weighted,
        ...
        for label in self.class_labels:
            for name_column in self.metrics_names[:-1]:
                self.df_dataset.loc[
                    index_patch, "_".join(label.split(" ")) + "_" + name_column
        ...
        for metric in self.metrics_names[:-1]:
            ...
            for class_name, weight in zip(self.class_labels, self.weights):
                mean_metric += (
                    weight
                    * self.df_dataset.loc[
                        index_patch, "_".join(class_name.split(" ")) + "_" + metric
                    ]
                )
            self.df_dataset.loc[index_patch, "mean_" + metric] = (
                mean_metric / self.nbr_class
            )
            if metric in ["Precision", "Recall", "F1-Score", "IoU"]:
                self.df_dataset.loc[index_patch, "macro_" + metric] = (
                    mean_metric / self.nbr_class
                )
    ...
    def metrics_to_df_reports(self):
        ...
        """
        for class_i in self.class_labels:
            self.df_report_classes.loc[class_i] = list(
                self.metrics_by_class[class_i].values()
            )[:-1]
        self.df_report_classes.loc["Overall"] = self.df_report_classes.mean()

        self.df_report_macro.loc["Average"] = [
            self.df_report_classes.loc["Overall", "Precision"],
        ...
        total_positive_obs = sum(
            [value for _, value in self.counts_sample_per_class.items()]
        )
        for metric in ["Precision", "Recall", "F1-Score", "IoU"]:
            ...
            for i, class_i in enumerate(self.class_labels):
                weighted_avg += (
                    self.df_report_classes.loc[class_i, metric]
                    * self.counts_sample_per_class[class_i]
                )

                if self.weighted:
                    user_weighted_avg += (
                        self.df_report_classes.loc[class_i, metric] * self.weights[i]
                    )

            self.df_report_macro.loc["Weighted avg", metric] = np.round(
                weighted_avg / total_positive_obs, decimals=self.decimals
            )

            if self.weighted:
                self.df_report_macro.loc["User weighted avg", metric] = np.round(
                    user_weighted_avg / self.nbr_class, decimals=self.decimals
                )

        # In micro : micro-F1 = micro-precision = micro-recall = accuracy
        self.df_report_micro.loc["Values"] = [
            self.metrics_micro["Precision"],
    ...
    def plot_dataset_metrics_histograms(self):
        ...
        for class_i in self.class_labels:
            ...
            output_paths[class_i] = plot_hists(
                hists=self.hists_metrics,
                list_metrics=self.header[header_index:header_next_index],
                output_path=osp.join(
                    self.output_path, "hists_" + "_".join(class_i.split(" ")) + ".png"
                ),
                n_bins=self.n_bins,
                bins_xticks=self.bins_xticks,
            )
    ...
    def export_values(self):
        ...
        if self.output_type == "json":
            ...
            if self.get_calibration_curves:
                ...
                self.dict_export["calibration curve"] = {
                    "prob_true": dict_prob_true,
