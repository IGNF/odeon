...
from odeon.metrics.metrics_module import IncrementalVariance, MeanVector
...
class StatsModule(pl.LightningModule):
    def __init__(self, num_channels, pixel_depth=255):
        ...
        self.report = pd.DataFrame(
            columns=["mean", "std"], index=["train", "val", "test"], dtype="object"
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.train_vars.update(
            images.transpose(0, 1).flatten(start_dim=1).transpose(0, 1)
        )

    def training_epoch_end(self, outputs):
        self.report.loc["train", "mean"] = (
            self.train_means.compute().cpu().numpy() / self.pixel_depth
        )
        self.report.loc["train", "std"] = (
            torch.sqrt(self.train_vars.compute()).cpu().numpy() / self.pixel_depth
        )

    def validation_step(self, batch, batch_idx):
        images = batch["image"]
        self.val_means.update(images.mean([0, 2, 3]))
        self.val_vars.update(
            images.transpose(0, 1).flatten(start_dim=1).transpose(0, 1)
        )

    def validation_epoch_end(self, outputs):
        self.report.loc["val", "mean"] = (
            self.val_means.compute().cpu().numpy() / self.pixel_depth
        )
        self.report.loc["val", "std"] = (
            torch.sqrt(self.val_vars.compute()).cpu().numpy() / self.pixel_depth
        )

    def test_step(self, batch, batch_idx):
        images = batch["image"]
        self.test_means.update(images.mean([0, 2, 3]))
        self.test_vars.update(
            images.transpose(0, 1).flatten(start_dim=1).transpose(0, 1)
        )

    def test_epoch_end(self, outputs):
        self.report.loc["test", "mean"] = (
            self.test_means.compute().cpu().numpy() / self.pixel_depth
        )
        self.report.loc["test", "std"] = (
            torch.sqrt(self.test_vars.compute()).cpu().numpy() / self.pixel_depth
        )
...
class Stats:
    def __init__(
        ...
        self.transforms = {
            phase: ToDoubleTensor() for phase in ["train", "val", "test"]
        }
    ...
    def __call__(self):
        try:
            self.trainer.fit(self.stats_module, datamodule=self.data_module)

            if self.data_module.test_file is not None:
                self.trainer.test(self.stats_module, datamodule=self.data_module)
